...
| `03_model_comparison.ipynb` | One block of three cells per model: its name, the fit — rolling-origin cross-validation as the score, Bayesian search (Optuna TPE) for the hyperparameters, out-of-fold permutation importance for the features — scored on the 2025 holdout, and the 2026 season it predicts: records, playoffs and champion. The first block also carries the harness the later ones reuse. |
